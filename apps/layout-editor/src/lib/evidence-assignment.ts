...
export function moveEvidenceRevealToCarrierInScene(
  ...
  const standaloneHotspotId = generatedStandaloneHotspotId(evidenceId);
  const existingStandaloneHotspot = scene.sublocations
    .flatMap((sublocation) => sublocation.hotspots)
    .find((hotspot) => hotspot.id === standaloneHotspotId);
  const standaloneTargetSublocationId =
    carrier?.kind === "standalone_hotspot" ? carrier.sublocationId : null;

  return {
    ...scene,
    sublocations: scene.sublocations.map((sublocation) => ({
      ...sublocation,
      hotspots: nextHotspotsForCarrier({
        scene,
        evidenceId,
        carrier,
        sublocationId: sublocation.id,
        existingStandaloneHotspot,
        standaloneTargetSublocationId,
...
function nextHotspotsForCarrier(options: {
  scene: InvestigationSceneJson;
  evidenceId: string;
  carrier: EvidenceCarrier | null;
  sublocationId: string;
  existingStandaloneHotspot:
    ...
    | undefined;
  standaloneTargetSublocationId: string | null;
  hotspots: InvestigationSceneJson["sublocations"][number]["hotspots"];
}): InvestigationSceneJson["sublocations"][number]["hotspots"] {
  const standaloneHotspotId = generatedStandaloneHotspotId(options.evidenceId);
  const hotspots = options.hotspots
    .filter(
      (hotspot) =>
        !(
          isGeneratedStandaloneHotspotId(hotspot.id) &&
          hotspot.id === standaloneHotspotId
        ),
    )
    .map((hotspot) => ({
      ...hotspot,
      reveals: nextRevealsForCarrier(
        hotspot.reveals,
        options.evidenceId,
        options.carrier?.kind === "hotspot" &&
          options.carrier.hotspotId === hotspot.id,
      ),
    }));

  if (options.standaloneTargetSublocationId !== options.sublocationId) {
    return hotspots;
  }

  return [
    ...hotspots,
    standaloneHotspotForEvidence(
      options.scene,
      options.evidenceId,
      options.existingStandaloneHotspot,
    ),
  ];
}

function standaloneHotspotForEvidence(
  scene: InvestigationSceneJson,
  evidenceId: string,
  existingHotspot:
    | InvestigationSceneJson["sublocations"][number]["hotspots"][number]
  ...
  return {
    id: generatedStandaloneHotspotId(evidenceId),
    label: existingHotspot?.label ?? evidence?.name ?? evidenceId,
    description: existingHotspot?.description ?? "編輯器產生的隱藏證據來源。",
    evidenceSource: existingHotspot?.evidenceSource ?? "hidden",
    sceneSourcePrompt:
      existingHotspot?.sceneSourcePrompt ??
    ...
    reveals: nextRevealsForCarrier(
      existingHotspot?.reveals ?? [],
      evidenceId,
      true,
    ),
    inspectDialogue: existingHotspot?.inspectDialogue ?? [],
    layout: existingHotspot?.layout ?? null,
  };
}
