...
`,
      ...
      (root) => {
        ...
        expect(
          result.errors.some(
            (e) => e.code === "assetCharacterMissingStandardExpression",
          ),
        ).toBe(true);
      },
    );
  ...
  it("rejects malformed character catalog shapes without throwing", () => {
    withConfig(
      ...
      (root) => {
        ...
        if (result.ok) return;
        expect(
          result.errors.some((e) => e.code === "assetCharactersMalformed"),
        ).toBe(true);
      },
    );
  });
...
`,
      ...
      (root) => {
        ...
        expect(
          result.errors.some(
            (e) => e.code === "assetCharacterExpressionMalformed",
          ),
        ).toBe(true);
      },
    );
...
`,
      ...
      (root) => {
        ...
        if (result.ok) return;
        expect(
          result.errors.some((e) => e.code === "assetCharacterDuplicateId"),
        ).toBe(true);
      },
    );
  });
...
`,
      ...
      (root) => {
        ...
        if (result.ok) return;
        expect(
          result.errors.some((e) => e.code === "assetCharacterIdMalformed"),
        ).toBe(true);
      },
    );
  });
...
`,
      ...
      (root) => {
        ...
        expect(
          result.errors.some(
            (e) => e.code === "assetCharacterExpressionIdMalformed",
          ),
        ).toBe(true);
      },
    );
...
`,
      ...
      (root) => {
        ...
        if (!result.ok) return;
        expect(
          result.value.characters.byId.get("draft")?.expressions.size,
        ).toBe(0);
      },
    );
  });
...
`,
      ...
      (root) => {
        ...
        if (result.ok) return;
        expect(
          result.errors.some((e) => e.code === "assetCharacterIdWrongType"),
        ).toBe(true);
      },
    );
  });
      ...
      (root) => {
        ...
        expect(
          result.warnings.some(
            (w) =>
              w.code === "assetConfigWrongType" &&
              w.message.includes('"displayNames"'),
          ),
        ).toBe(true);
...
`,
      ...
      (root) => {
        ...
        expect(
          result.errors.some(
            (e) =>
              e.code === "assetPolicyUnsupportedFormat" &&
              e.message.includes("webp"),
          ),
        ).toBe(true);
      },
    );
...
`,
      ...
      (root) => {
        ...
        expect(result.value.enabled).toBe(false);
        expect(
          result.warnings.some((w) => w.code === "assetPolicyMissing"),
        ).toBe(true);
      ...
      (root) => {
        ...
        expect(
          result.warnings.some(
            (w) =>
              w.code === "assetConfigWrongType" &&
              w.message.includes('"types.background.transparency"'),
          ),
        ).toBe(true);
      ...
      (root) => {
        ...
        expect(
          result.warnings.some(
            (w) =>
              w.code === "assetConfigWrongType" &&
              w.message.includes('"types.audio.loop"'),
          ),
        ).toBe(true);
...
`,
      ...
      (root) => {
        ...
        if (!result.ok) return;
        expect(
          result.warnings.some((w) => w.message.includes("transparency")),
        ).toBe(false);
      },
    );
  });
      ...
      (root) => {
        ...
        expect(
          result.warnings.some(
            (w) =>
              w.code === "assetConfigWrongType" &&
              w.message.includes('"types.background.dimensions"'),
          ),
        ).toBe(true);
      ...
      (root) => {
        ...
        expect(
          result.warnings.some(
            (w) =>
              w.code === "assetConfigWrongType" &&
              w.message.includes('"types.portrait.dimensions"'),
          ),
        ).toBe(true);
