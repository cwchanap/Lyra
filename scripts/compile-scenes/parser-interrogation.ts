// =============================================================================
// scripts/compile-scenes/parser-interrogation.ts
//
...
export function parseInterrogationScene(
  ...
  if (!first || first.kind !== "heading" || first.level !== 1) {
    return fail(sourceFile, first?.line ?? 1, "interrogationSceneMissingTitle", "Interrogation scene must start with `# Scene N: <title>`.");
  }
  cur.next();
  const titleMatch = /^Scene\s+\d+:\s*(.+)$/.exec(first.text);
  if (!titleMatch) {
    return fail(sourceFile, first.line, "interrogationSceneMissingTitle", "Interrogation scene must start with `# Scene N: <title>`.");
  }
  ...
  while (!cur.done()) {
    ...
    if (tok.kind !== "heading" || tok.level !== 2) {
      return fail(sourceFile, tok.line, "interrogationSceneUnknownH2", `Expected H2 block heading at scene top level; got: ${describe(tok)}.`);
    }
    ...
    if (tok.text === "Intro") {
      ...
      return fail(sourceFile, tok.line, "interrogationSceneUnknownH2", `Unknown H2 heading: ${tok.text}.`);
    }
  }
...
function parsePhase(cur: Cursor): { ok: true; value: ASTInterrogationPhase } | { ok: false; error: CompileError } {
  const head = cur.next();
  if (!head || head.kind !== "heading" || head.level !== 2) return fail(cur.sourceFile, head?.line ?? 1, "internalParserState", "parsePhase called off-position.");
  if (!head.anchorId) return fail(cur.sourceFile, head.line, "interrogationPhaseMissingAnchor", "Phase heading must include {#id}.");
  const labelMatch = /^Phase:\s*(.+)$/.exec(head.text);
  if (!labelMatch) return fail(cur.sourceFile, head.line, "interrogationPhaseMalformedHeading", `Malformed phase heading: ${head.text}`);
  ...
  if (kind === "testimony") return parseTestimonyPhase(cur, { head, id: head.anchorId, label, meta: meta.value });
  return fail(cur.sourceFile, head.line, "interrogationPhaseUnknownKind", `Unknown interrogation phase Kind: ${kind}.`);
}
...
function parseSubject(cur: Cursor): { ok: true; value: ASTSubject } | { ok: false; error: CompileError } {
  const head = cur.next();
  if (!head || head.kind !== "heading" || head.level !== 3) return fail(cur.sourceFile, head?.line ?? 1, "internalParserState", "parseSubject called off-position.");
  if (!head.anchorId) return fail(cur.sourceFile, head.line, "interrogationSubjectMissingAnchor", "Subject heading must include {#id}.");
...
function parseInquiryPhase(cur: Cursor, phaseMeta: PhaseMeta): { ok: true; value: ASTInquiryPhase } | { ok: false; error: CompileError } {
  ...
  while (true) {
    ...
    if (next.kind === "heading" && next.level === 3 && next.text.startsWith("Question:")) {
    ...
    if (next.kind === "heading" && next.level === 4 && next.text.startsWith("Follow-up:")) {
      if (!lastQuestionId) return fail(cur.sourceFile, next.line, "interrogationQuestionMalformedHeading", "Follow-up must appear after a parent Question.");
      const q = parseInquiryQuestion(cur, lastQuestionId);
      if (!q.ok) return q;
      questions.push(q.value);
      continue;
    }
    const body = consumePhaseBodyToken(cur, sceneTag, entryDialogue, phaseMeta.id);
...
function parseTestimonyPhase(cur: Cursor, phaseMeta: PhaseMeta): { ok: true; value: ASTTestimonyPhase } | { ok: false; error: CompileError } {
  const common = parseCommonPhaseMeta(phaseMeta);
  if (!common.ok) return common;
  ...
  while (true) {
    ...
    if (next.kind === "heading" && next.level === 3 && next.text === "Testimony") {
      ...
      while (true) {
        ...
        if (tok.kind === "heading" && tok.level === 4 && tok.text.startsWith("Statement:")) {
          const s = parseTestimonyStatement(cur);
          if (!s.ok) return s;
          statements.push(s.value);
          continue;
        }
        return fail(cur.sourceFile, tok.line, "testimonyMissingContainer", `Unexpected content in Testimony container: ${describe(tok)}.`);
      }
      continue;
    }
    if (next.kind === "heading" && next.level === 3 && next.text.startsWith("Result:")) {
...
function parseInquiryQuestion(
...
): { ok: true; value: ASTInquiryQuestion } | { ok: false; error: CompileError } {
  const head = cur.next();
  if (!head || head.kind !== "heading") return fail(cur.sourceFile, head?.line ?? 1, "internalParserState", "parseInquiryQuestion called off-position.");
  if (!head.anchorId) return fail(cur.sourceFile, head.line, "interrogationQuestionMissingAnchor", "Question heading must include {#id}.");
  const expectedLevel = parentQuestionId === null ? 3 : 4;
  if (head.level !== expectedLevel) return fail(cur.sourceFile, head.line, "interrogationQuestionMalformedHeading", `Unexpected question heading level H${head.level}.`);
  const labelMatch = parentQuestionId === null
    ? /^Question:\s*(.+)$/.exec(head.text)
    : /^Follow-up:\s*(.+)$/.exec(head.text);
  if (!labelMatch) return fail(cur.sourceFile, head.line, "interrogationQuestionMalformedHeading", `Malformed question heading: ${head.text}`);
  ...
  const required = parseBoolean(meta.value.Required, true, cur.sourceFile, head.line);
  ...
  if (next && next.kind === "heading" && next.level === onReaskLevel && next.text === "On Reask") {
    cur.next();
    const r = consumeDialogueUntilHeading(cur, onReaskLevel);
    if (!r.ok) return r;
    onReask = r.value;
  } else if (next && next.kind === "heading" && next.level === 5 && parentQuestionId === null) {
    return fail(cur.sourceFile, next.line, "interrogationQuestionMalformedHeading", `Unexpected H5 under Question: ${next.text}.`);
  }
...
function parseTestimonyStatement(cur: Cursor): { ok: true; value: ASTTestimonyStatement } | { ok: false; error: CompileError } {
  const head = cur.next();
  if (!head || head.kind !== "heading" || head.level !== 4) return fail(cur.sourceFile, head?.line ?? 1, "internalParserState", "parseTestimonyStatement called off-position.");
  if (!head.anchorId) return fail(cur.sourceFile, head.line, "testimonyStatementMissingAnchor", "Testimony statement heading must include {#id}.");
  const labelMatch = /^Statement:\s*(.+)$/.exec(head.text);
  if (!labelMatch) return fail(cur.sourceFile, head.line, "testimonyStatementMissingAnchor", `Malformed testimony statement heading: ${head.text}`);
  ...
  while (true) {
    ...
    if (next.kind === "heading" && next.level === 5) {
      ...
      return fail(cur.sourceFile, next.line, "testimonyUnknownH5", `Unknown H5 under testimony statement ${head.anchorId}: ${next.text}.`);
    }
    return fail(cur.sourceFile, next.line, "testimonyUnknownH5", `Unexpected token in testimony statement ${head.anchorId}: ${describe(next)}.`);
  }
...
function parseTestimonyResult(cur: Cursor): { ok: true; value: ASTTestimonyResult } | { ok: false; error: CompileError } {
  const head = cur.next();
  if (!head || head.kind !== "heading" || head.level !== 3) return fail(cur.sourceFile, head?.line ?? 1, "internalParserState", "parseTestimonyResult called off-position.");
  if (!head.anchorId) return fail(cur.sourceFile, head.line, "testimonyResultMissingAnchor", "Testimony result heading must include {#id}.");
...
function parseOutro(cur: Cursor): { ok: true; value: ASTInterrogationOutro } | { ok: false; error: CompileError } {
  const head = cur.next();
...
function consumePhaseBodyToken(
  ...
  phaseId: string,
): { ok: true; value: { sceneTag: string | null } } | { ok: false; error: CompileError } {
  const next = cur.next();
  if (!next) return { ok: true, value: { sceneTag } };
  if (next.kind === "sceneTag") {
    if (sceneTag !== null) return fail(cur.sourceFile, next.line, "interrogationPhaseDuplicateSceneTag", `Phase ${phaseId} declared multiple [場景：...] tags.`);
  ...
  if (next.kind === "unknown") return fail(cur.sourceFile, next.line, "unrecognizedDialogueLine", `Unrecognized line in phase body: ${next.text}.`);
  return fail(cur.sourceFile, next.line, "interrogationPhaseMalformedHeading", `Unexpected H${next.level} "${next.text}" in phase ${phaseId}.`);
}
...
function parseInterrogationRevealsList(
  ...
  sourceFile: string,
  line: number,
): { ok: true; value: InterrogationRevealTarget[] } | { ok: false; error: CompileError } {
  const m = /^\[(.*)\]\s*$/.exec(raw.trim());
  if (!m) return fail(sourceFile, line, "interrogationRevealUnknownTarget", `Reveals value must be a [list]. Got: ${raw}`);
...
function parseInterrogationRevealTarget(
  ...
  if (!m) return fail(sourceFile, line, "interrogationRevealUnknownTarget", `Unknown interrogation reveal target: ${raw}`);
...
function parseInventoryTarget(
  ...
  if (!m) return fail(sourceFile, line, "interrogationContradictionMalformed", `Contradiction must be evidence:<id> or statement:<id>. Got: ${raw}`);
  return { ok: true, value: { kind: m[1] as InventoryTarget["kind"], id: m[2] ?? "" } };
}
...
function parseBoolean(
  ...
  return fail(sourceFile, line, "invalidBooleanValue", `Boolean metadata must be "true" or "false"; got "${raw}".`);
}
...
function validateStatus(
  ...
  return fail(sourceFile, line, "invalidStatusValue", `Status must be "locked" or "unlocked"; got "${raw}".`);
}
